  ...
  return (
    <div>
      ...
      <section className="mb-8">
        <h2 className="font-semibold text-gray-700 dark:text-gray-200 text-sm uppercase tracking-wide mb-3">
          Promotion Requests
        </h2>
        {promotions.length === 0 ? (
            ...
            {promotions.map((req) => {
              ...
              return (
                <div key={req.id} className={`rounded-xl border border-gray-200 dark:border-gray-800 p-5 transition-shadow hover:shadow-md ${cardTheme(req.status)}`}>
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-sm text-gray-500 dark:text-gray-300">{req.team.name}</span>
                      </div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-300 mt-2 italic">&quot;{req.justification}&quot;</p>
                    </div>
                  ...
                  {req.reviewNote && (
                    <p className="text-xs text-gray-500 dark:text-gray-300 mt-3 border-t border-gray-100 dark:border-gray-800 pt-2">
                  ...
                  {req.status === 'PENDING' && !showReview && isMine && (
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-3 border-t border-gray-100 dark:border-gray-800 pt-2">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                        ...
                        <span className="text-gray-400 dark:text-gray-400 ml-1 text-xs">(+{pct}%)</span>
                      </p>
                      <p className="text-xs text-gray-400 dark:text-gray-400 mt-1">
                        Recommended by {isMine ? 'you' : req.recommender?.name ?? 'Former user'} · {format(req.createdAt, 'MMM d, yyyy')}
                      </p>
