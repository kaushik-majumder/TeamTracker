    ...
    <div>
      <div className="mb-6">
        ...
        <div className="flex items-start justify-between mt-2">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">{team._count.employees} employees</p>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-900 mb-3">Assigned Users</h2>
          {team.teamAccess.length === 0 ? (
            <p className="text-sm text-gray-400">No users assigned yet.</p>
          ) : (
            <div className="space-y-2">
              {team.teamAccess.map((access) => (
                <div key={access.id} className="flex items-center justify-between border-b border-gray-100 pb-2 last:border-0">
                  <div>
                    <p className="text-sm font-medium text-gray-800">{access.user.name}</p>
                    <p className="text-xs text-gray-500">{access.user.email}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      access.role === 'MANAGER' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'
                    }`}>
        ...
        {/* Assign new */}
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-900 mb-3">Assign User</h2>
