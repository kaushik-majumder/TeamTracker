...
export function CreateCycleForm({ teams }: Props) {
  ...
  return (
    <form action={action} className="space-y-3" key={s?.success ? 'reset' : 'form'}>
      <div>
        <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Cycle Name</label>
        <input
          name="name"
          placeholder="Q1 2026"
      ...
      <div>
        <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">
          Description <span className="text-gray-400 dark:text-gray-400 font-normal">(optional)</span>
        </label>
        <textarea
          name="description"
          rows={2}
      ...
      <div>
        <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-2">Scope</label>
        ...
        <div className="space-y-2 mb-2">
          {(['ALL', 'SELECTED'] as const).map((mode) => (
            <label key={mode} className="flex items-center gap-2 text-sm cursor-pointer">
              ...
              />
              <span className="text-gray-700 dark:text-gray-200">{mode === 'ALL' ? 'All teams' : 'Specific teams'}</span>
            </label>
          ))}
        </div>
        ...
        {scope === 'SELECTED' && (
          <div className="max-h-32 overflow-y-auto border border-gray-200 dark:border-gray-800 rounded-lg p-2 space-y-1">
            {teams.length === 0 ? (
              <p className="text-xs text-gray-500 dark:text-gray-300 p-2">No teams yet — create teams first.</p>
            ) : (
              teams.map((t) => (
