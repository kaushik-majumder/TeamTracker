...
export default async function TeamDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href="/dashboard/teams" className="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-200">
          ← Teams
        </Link>
              ...
              return (
                <div key={access.id} className="flex items-center justify-between gap-3 text-sm">
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="text-gray-500 dark:text-gray-300 ml-2">{access.user.email}</span>
                  </div>
      ...
      <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">
        Members <span className="text-gray-400 dark:text-gray-400 font-normal">({team.employees.length})</span>
      </h2>
          ...
          {team.employees.map((emp) => {
            ...
            return (
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    <span className={`font-medium ${isActive ? 'text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-300'}`}>
                      {emp.name}
                    </span>
                    {isAnniversaryMonth && (
                    ...
                    {!isActive && (
                      <span className="text-xs bg-gray-200 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-full">
                        Former
                      </span>
                    )}
                  </div>
                  <p className="text-sm text-gray-500 dark:text-gray-300 mt-0.5">{emp.title}</p>
                </div>
