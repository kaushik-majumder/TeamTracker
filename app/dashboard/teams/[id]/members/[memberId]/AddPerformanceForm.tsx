    ...
    <form ref={formRef} action={action} className="space-y-3">
      ...
      <div>
        <label className="block text-xs font-medium text-gray-600 mb-1">
          Attachments <span className="text-gray-400 font-normal">(optional, max 3MB each)</span>
        </label>

        {files.length > 0 && (
          <ul className="space-y-1 mb-2">
            {files.map((f, i) => (
              <li key={i} className="flex items-center justify-between text-xs bg-gray-50 rounded-lg px-2 py-1.5">
        ...
        <label className="inline-flex items-center gap-1.5 text-xs bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium px-2.5 py-1.5 rounded-lg cursor-pointer transition-colors">
          <Paperclip size={12} />
